...
* **Purpose:** This anchor provides a stable slot in the Logical View (`LView`) to store the component's metadata, its `NodeInjector`, and the `setup()` closure results. It also holds the **Directive Bag** — the opaque collection of directive definitions supplied by the parent via the `attachments` sink — so the engine can retrieve and instantiate them at the `{...attachments()}` spread site.
...
### 5. Directive Sinks & Content Projection
Ambient metadata — directives and projected content — flows through explicit sinks rather than implicit host attachment or `<ng-content>` slots.
* **`attachments` (Directive Sink):** Directives applied to a component's selector are validated at compile time against the element type declared in `directives<T>()`. The `{...attachments()}` spread is not a runtime object spread; the compiler emits a `ɵɵapplyDirectiveSink(slot, attachments)` instruction instead. At runtime, this instruction reads the directive definitions from the Directive Bag and instantiates them on the target DOM element. This preserves **isolated AOT compilation** — the child template is compiled without knowledge of which directives the parent will supply; only the element-type constraint is known at build time.
...
| **Directives** | Automatically attach to the host element. | Collected in the Directive Bag and instantiated at the `{...attachments()}` site via `ɵɵapplyDirectiveSink`. |
